...
export const create = async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, redirectTo: "/admin/category", message: "Created successfully" });
  } catch (err) {
    console.log(err)
...
export const destroy = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Deleted successfully', redirectTo: "/admin/category" });
  } catch (err) {
    console.log(err)
    res.status(500).json({ error: err });
  }
